...
describe('Meal Agent', () => {
    ...
    describe('Add Meal', () => {
        it('should add meal without food successfully', async () => {
            let result = await agent.addMeal(correctRequestObj);
        ...
        it('should add meal with food successfully', async () => {
            let food = await foodAgent.addFood(foodRequestObj);
            let result = await agent.addMeal(correctRequestObj, [food]);
            expect(result.id).to.be.equal(1);
        ...
        it('should reject meal with wrong type', async() => {
            ...
            expect(agent.addMeal(requestObj)).to.be.rejected;
        });
    });
    ...
    describe('Get Meal', async () => {
        it('should get all meals', async () => {
            let food = await foodAgent.addFood(foodRequestObj);
            await agent.addMeal(correctRequestObj, [food]);
            let result = await agent.getMeals();
        ...
        it('should get single meal with id', async () => {
            let food = await foodAgent.addFood(foodRequestObj);
            await agent.addMeal(correctRequestObj, [food]);
            let result = await agent.getMeal(1);
            expect(result.type).to.be.equal(correctRequestObj.type);
            expect(result.foods[0].name).to.be.equal(foodRequestObj.name);
        });

        it('should reject wrong id', async () => {
            expect(agent.getMeal(1000)).to.be.rejected;
        });

        it('should get meals with ids', async () => {
            await agent.addMeal(correctRequestObj);
            await agent.addMeal(correctRequestObj);
            await agent.addMeal(correctRequestObj);
            let result = await agent.getMealsWithIds([1, 3]);
    ...
    describe('Update Meal', async () => {
        it('should update meal successfully', async () => {
            await agent.addMeal(correctRequestObj);
            let food = await foodAgent.addFood(foodRequestObj);
            let result = await agent.updateMeal(1, {type: MealType.Breakfast}, [food]);
            expect(result.id).to.be.equal(1);
        ...
        it('should reject wrong id', async() => {
            expect(agent.updateMeal(1000, {})).to.be.rejected;
        });
    });
    ...
    describe('Delete Meal', async () => {
        it('should delete a meal', async () => {
            await agent.addMeal(correctRequestObj);
            let result = await agent.deleteMeal(1);
            expect(result.type).to.be.equal(correctRequestObj.type);
            expect(agent.getMeal(1)).to.be.rejected;
        });

        it('should reject deletion for wrong id', async () => {
            expect(agent.deleteMeal(1000)).to.be.rejected;
        });
    });
    ...
    describe('Food in Meal', () => {
        ...
        beforeEach(async () => {
            await agent.addMeal(correctRequestObj);
            food = await foodAgent.addFood(foodRequestObj);
            result = await agent.addFoodToMeal(1, [food]);
        });
        ...
        describe('Add Food to Meal', async() => {
            ...
            it('no duplicate food should exist on meal', async () => {
                await agent.addFoodToMeal(1, [food]);
                let result = await agent.getMeal(1);
                expect(result.foods.length).to.be.equal(1);
            });
        });
        ...
        describe('Remove Food from Meal', async() => {
            it('should remove food from meal', async () => {
                let result = await agent.removeFoodFromMeal(1, food);
                expect(result.foods.length).to.be.equal(0);
            });

            it('should reject removing food that isn\'t in meal', async () => {
                let otherFood = await foodAgent.addFood({type: FoodType.Protein, name: 'Sample', price: 20});
                expect(agent.removeFoodFromMeal(1, otherFood)).to.be.rejected;
            });
        });
    });
